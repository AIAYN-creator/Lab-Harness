...
|---|---|
| `labharness init [PATH] [--journal NAME]` | Create a workspace from a template: `acs`, `elsevier`, `rsc-draft`, `article` or `report` |
...
- [x] CI on Linux and Windows, and macOS as a second tier; protected `main` and release tags; security alerts
- [x] `doctor` prints the exact command that fixes each problem
- [ ] `init --journal ... --font ...`, and a test that each combination embeds a single typeface
- [ ] Publishing to PyPI, so `pip install labharness` works
- [ ] A gallery of examples, a full command reference and a demo GIF

Nothing here is dated: see [`docs/ROADMAP.md`](docs/ROADMAP.md).

**Not planned before v2.0:** a graphical interface.
