...
> references from a DOI, protected raw data and `eject`. Everything described below runs on
> `main` today; see [Progress](#progress) for what is done and what is next.
...
```
 data/                     scripts/                  figures/ tables/    paper.pdf
 raw measurements   ---->  transparent scripts ----> vector figures ---> your manuscript
 .csv .xlsx .smi           .py and .tex              and LaTeX tables
...
- **Local-first.** Everything runs on your machine. The one command that goes online,
  `labharness cite`, does so only when you run it.
...
| `library` | `chem` and `excel` | The lab compound inventory |
...
| `labharness library check` | Review the lab compound library: unreadable SMILES, missing stereochemistry, wrong formulas |
...
**[How to use each command, the manifest and the file formats: `docs/usage.md`](docs/usage.md)**
· **[Every option of every command: `docs/cli.md`](docs/cli.md)**
...
table = read_table("data/optimisation.csv")  # CSV, TSV, TXT or .xlsx, every cell as written
...
- **One reader for every file**, plots and tables alike. It reads what old instrument software
  writes (UTF-16, the Windows encoding, `;` and decimal commas, metadata above the header) and
  Excel workbooks, and never guesses in silence: a bad cell stops with its file, line and column.
- **Axis labels from the header**: a column called `t (min)`, `t [min]` or `t / min` gives its
  axis the label t (min). A header without a unit is refused, not drawn without one.
- **The lab's compound library**: an inventory exported from Access or Excel, named in the
  manifest. `labharness resolve CAT-7B` and `render_structure(compound="CAT-7B", ...)` use it,
  and `labharness library check` flags chiral compounds without stereochemistry and formulas that
  do not match their SMILES. Nothing in the inventory is ever changed.
- **References from a DOI**: `labharness cite 10.1016/...` adds the work to `references.bib` and
  prints the `\cite{...}` for it.
...
- [x] Reading instrument exports: encodings, separators, decimal commas, metadata rows, units in
  the header, and Excel
- [x] A lab compound library, so a figure can use the compound's in-house code
- [x] Tables rebuilt by the watcher, and `labharness add table`
- [x] Data errors that name the row and column, without stopping the watcher
- [x] Fitted values and uncertainties rounded together, the same way in the text and in tables
- [ ] Tested against real exports from the lab's instruments
- [ ] Chromatograms and spectra from any instrument's export, with integration windows
- [ ] A chapter of the thesis built with it: the checkpoint that closes v0.5
...
- [x] `init --journal ... --font ...`
- [x] A test that every journal and typeface combination embeds a single typeface
...
- [x] `labharness cite DOI`
- [x] A full command reference, generated from the command line

**v1.5 — modular, planned.** Anyone can build a package for their field without touching the
core.

- [ ] Domains registered as packages, with their own figure templates and agent rules
- [ ] `labharness new-domain NAME`, a package ready to fill in
- [ ] `labharness open`: manuscript, console and PDF side by side, the PDF on either side
