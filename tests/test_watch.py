...
def test_tikz_figures_are_not_implemented_yet(tmp_path: Path) -> None:
    (tmp_path / "labharness.toml").write_text(
        '[[figure]]\noutput = "figures/m.pdf"\nscript = "scripts/m.tex"\n', encoding="utf-8"
    )
    (tmp_path / "scripts").mkdir()
    (tmp_path / "scripts" / "m.tex").write_text("% diagram", encoding="utf-8")
    workspace = load_workspace(tmp_path)

    result = build_figure(workspace, workspace.figures[0])

    assert not result.ok
    assert "not implemented yet" in (result.error or "")
