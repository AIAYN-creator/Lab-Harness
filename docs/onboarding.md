...
1. Copy `src/labharness/style/journals/acs.toml` and adjust the values.
2. Add `templates/workspace/journals/<journal>/` with its `paper.tex`, `references.bib` and a
   `labharness-style.tex` generated by `labharness.style.latex.document_preamble`.
3. Nothing else: no module should need changing. If one does, that is a bug in the module.
